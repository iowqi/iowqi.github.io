import Key from "../i18nKey";
import type { Translation } from "../translation";

export const th: Translation = {
	[Key.home]: "หน้าแรก",
	[Key.about]: "เกี่ยวกับ",
	[Key.archive]: "คลัง",
	[Key.search]: "ค้นหา",

	[Key.tags]: "ป้ายกำกับ",
	[Key.categories]: "หมวดหมู่",
	[Key.recentPosts]: "โพสต์ล่าสุด",

	[Key.comments]: "ความคิดเห็น",

	[Key.untitled]: "ไม่ได้ตั้งชื่อ",
	[Key.uncategorized]: "ไม่ได้จัดหมวดหมู่",
	[Key.noTags]: "ไม่มีป้ายกำกับ",

	[Key.wordCount]: "คำ",
	[Key.wordsCount]: "คำ",
	[Key.minuteCount]: "นาที",
	[Key.minutesCount]: "นาที",
	[Key.postCount]: "โพสต์",
	[Key.postsCount]: "โพสต์",

	[Key.themeColor]: "สีของธีม",
	[Key.postLayout]: "เลย์เอาต์บทความ",
	[Key.postLayoutList]: "รายการ",
	[Key.postLayoutGrid]: "กริด",

	[Key.lightMode]: "สว่าง",
	[Key.darkMode]: "มืด",
	[Key.systemMode]: "ตามระบบ",

	[Key.more]: "ดูเพิ่ม",

	[Key.author]: "ผู้เขียน",
	[Key.publishedAt]: "เผยแพร่เมื่อ",
	[Key.license]: "สัญญาอนุญาต",

	[Key.calendar]: "วันที่",
	[Key.calendarJanuary]: "ม.ค.",
	[Key.calendarFebruary]: "ก.พ.",
	[Key.calendarMarch]: "มี.ค.",
	[Key.calendarApril]: "เม.ย.",
	[Key.calendarMay]: "พ.ค.",
	[Key.calendarJune]: "มิ.ย.",
	[Key.calendarJuly]: "ก.ค.",
	[Key.calendarAugust]: "ส.ค.",
	[Key.calendarSeptember]: "ก.ย.",
	[Key.calendarOctober]: "ต.ค.",
	[Key.calendarNovember]: "พ.ย.",
	[Key.calendarDecember]: "ธ.ค.",
	[Key.calendarSunday]: "อา.",
	[Key.calendarMonday]: "จ.",
	[Key.calendarTuesday]: "อ.",
	[Key.calendarWednesday]: "พ.",
	[Key.calendarThursday]: "พฤ.",
	[Key.calendarFriday]: "ศ.",
	[Key.calendarSaturday]: "ส.",
	[Key.year]: "",
	[Key.calendarClearFilter]: "ล้างตัวกรองวันที่",
	[Key.calendarDragHint]:
		"ลากบนวันที่เพื่อกรองรายการในหน้าแรก ลากออกเหนือหรือใต้ปฏิทินเพื่อเปลี่ยนเดือนขณะเลือก หรือคลิกวันที่หนึ่ง แล้วกด Shift คลิกอีกวันเพื่อเลือกช่วงระหว่างสองวัน",
	[Key.calendarPrevMonth]: "เดือนก่อน",
	[Key.calendarNextMonth]: "เดือนถัดไป",
	[Key.calendarBackToday]: "กลับไปวันนี้",
};
